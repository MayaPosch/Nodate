...
bool GPIO::set_output(GPIO_ports port, uint8_t pin, GPIO_pupd pupd, GPIO_out_type type, 
	...
	*((uint32_t*) &(instance.regs->GPIO0_CTRL) + (2 * pin)) |= (0x3 << IO_BANK0_GPIO0_CTRL_OEOVER_Pos);
...
bool GPIO::write(GPIO_ports port, uint8_t pin, GPIO_level level) {
	...
	if (level == GPIO_LEVEL_LOW) {
		...
		*((uint32_t*) &(instance.regs->GPIO0_CTRL) + (2 * pin)) &= ~(0x3 << IO_BANK0_GPIO0_CTRL_OUTOVER_Pos);
		*((uint32_t*) &(instance.regs->GPIO0_CTRL) + (2 * pin)) |= (0x2 << IO_BANK0_GPIO0_CTRL_OUTOVER_Pos);
	}
